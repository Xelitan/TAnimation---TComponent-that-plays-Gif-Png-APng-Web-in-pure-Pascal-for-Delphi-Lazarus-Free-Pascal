...
//		All rights reserved.                                          //
//                                                                            //
// Drop a TAnimation on a form, set FileName (or call LoadFromFile /          //
...
type
  ...
  { TAnimation }
  TAnimation = class(TGraphicControl)
  private
...
procedure TAnimation.ReplaceDecoder(Fmt: TAnimFormat);
begin
  FreeAndNil(FAnim);
  case Fmt of
    ...
    raise EInvalidGraphic.Create('TAnimation: unrecognised image format');
  end;
end;
...
procedure TAnimation.SizeToFrame;
begin
  // When not stretching, match the control to the animation canvas.
  if (FAnim <> nil) and (not FStretch) and
     (FAnim.Width > 0) and (FAnim.Height > 0) then
...
procedure TAnimation.LoadFromStream(Stream: TStream);
begin
  FTimer.Enabled := False;
...
procedure TAnimation.LoadFromFile(const AFileName: String);
var fs: TFileStream;
begin
...
procedure TAnimation.SetFileName(const Value: String);
begin
  FFileName := Value;
...
procedure TAnimation.SetFrameIndex(Value: Integer);
begin
...
procedure TAnimation.SetStretch(Value: Boolean);
begin
...
procedure TAnimation.ScheduleNext;
var ms: Integer;
begin
  if GetFrameCount = 0 then Exit;
...
begin
  ...
  begin
    ...
    r := ClientRect;
    Canvas.Rectangle(r);
    Canvas.TextOut(4, 4, 'TAnimation');
  end;
end;

procedure Register;
begin
  RegisterComponents('Xelitan', [TAnimation]);
end;
